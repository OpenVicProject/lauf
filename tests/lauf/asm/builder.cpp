...
TEST_CASE("lauf_asm_build_string_literal")
{
    ...
    auto mut_abc = lauf_asm_add_global(mod, LAUF_ASM_GLOBAL_READ_WRITE);
    lauf_asm_define_data_global(mod, mut_abc, {4, 1}, "abc");

    auto no_null_abc = lauf_asm_add_global(mod, LAUF_ASM_GLOBAL_READ_ONLY);
    lauf_asm_define_data_global(mod, no_null_abc, {3, 1}, "abc");
...
{
    auto build_bytes = [](lauf_uint value) {
        return build({0, 1}, [=](lauf_asm_module*, lauf_asm_builder* b) {
            lauf_asm_inst_bytes(b, &value);
        });
    };
...
TEST_CASE("lauf_asm_inst_global_addr")
{
    auto single = build({0, 1}, [](lauf_asm_module* mod, lauf_asm_builder* b) {
        auto glob = lauf_asm_add_global(mod, LAUF_ASM_GLOBAL_READ_WRITE);
        lauf_asm_define_data_global(mod, glob, {42, 1}, nullptr);
